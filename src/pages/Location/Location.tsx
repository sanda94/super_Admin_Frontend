...
const Location: React.FC = () => {
  ...
  const FetchData = async () => {
    try {
      ...
      if (UserType === "SuperAdmin") {
        ...
        url = `${baseUrl}/locations/company/${CompanyId}`;
      }
      ...
      if (response.data.status) {
        const dataResponse = response.data.locations;

        const transformLocations = dataResponse.map((location: any) => {
          const company = companies.find(
            (comp) => comp._id === location.companyId
          );
          return {
            ...location,
        ...
        setLocationData(transformLocations);
      }
      // else{
  ...
  const CreateLocation = async () => {
    ...
    try {
      const response = await axios.post(`${baseUrl}/locations/create`, data, {
        headers: {
          token: `Bearer ${Token}`,
        },
      });
  ...
  // Define columns for Locations page
  ...
  return (
    <div className="z-[100]">
      <div className="flex items-center justify-between gap-10 lg:justify-start">
        <PageHeader title="LOCATIONS MANAGEMENT" subTitle="" />
